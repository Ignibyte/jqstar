...
| `jquery-star/inspect`          | ESM, CommonJS      | Explicit inspection attachment; tracing disabled by default.          |
...
`jquery-star/stores`, `jquery-star/persist`, and `jquery-star/inspect` are stable in 1.1. The
inspection data schemas are versioned independently; see [INSPECTION.md](INSPECTION.md).
